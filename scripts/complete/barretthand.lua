...
function sysCall_init() 
    ...
    closing=false
    handLocked = false -- ADD THIS NEW VARIABLE
    sliderV=50
...
function sysCall_actuation()
    ...
    if handLocked then
        -- If locked, stop all joint motors
        for i=1,3,1 do
            sim.setJointTargetVelocity(jointHandles[i][2], 0)
            sim.setJointTargetVelocity(jointHandles[i][3], 0)
        end
